...
static void handleUsbEvent(struct uevent *evt)
{
    ...
    //ÈçÏÂÅÐ¶ÏÉè±žÀàÐÍ£¬ºÍÊÇ·ñÎªaddÄ£Êœ¡£ œøÐÐÏàÓŠ²Ù×÷
...
*/
        ...
        p += sizeof("usb");
        /*Èç¹ûÊÇusb¿ØÖÆÆ÷ÔòÉÏ±šÀàËÆÈçÏÂpath£º  /devices/platform/sw-ehci.1/usb*
          Èç¹ûÊÇÍâÉè²åÈëÔòÉÏ±šÀàËÆÈçÏÂpath£º   /devices/platform/sw-ehci.1/usb1/1-1/1-1.7
        */
        p = strchr(p,'-');
        if(p == NULL)
        {
...
static void on_uevent(struct uevent *event)
{
	...
	if (!strcmp(subsys, "usb")) {
    	handleUsbEvent(event);	//ŽËº¯ÊýÐèÒªÔÚ EventÀàÖÐÌíŒÓ
    }
