...
DBG_TIME_AVG_BEGIN(TAG_PREVIEW);
	  ...
	  ,mFaceDetectionStart(false)
{
	...
	pthread_mutex_init(&mQueueEmptyMutex, NULL);
	pthread_cond_init(&mQueueEmptyCond, NULL);
	
	pthread_mutex_init(&mPreviewSwitchMutex, NULL);
	pthread_cond_init(&mPreviewSwitchCond, NULL);
...
V4L2CameraDevice::~V4L2CameraDevice()
{
	...
	pthread_mutex_destroy(&mQueueEmptyMutex);
	pthread_cond_destroy(&mQueueEmptyCond);
	
	pthread_mutex_destroy(&mPreviewSwitchMutex);
	pthread_cond_destroy(&mPreviewSwitchCond);
}
...
status_t V4L2CameraDevice::disconnectDevice()
{
	...
	DBG_TIME_BEGIN("close camera", 1);
	
	// close v4l2 camera device
	closeCameraDev();
	
	DBG_TIME_END("close camera", 1);
                                       ...
                                       bool video_hint)
{
	...
	if (mFrameWidth * mFrameHeight >= 1280 * 720)
	{
		mPreviewAfter = 1000000 / 20;
	}

	if (0 && video_hint && (mCameraFacing == CAMERA_FACING_BACK))
	{
		LOGV("use hw preview: %d x %d", mFrameWidth, mFrameHeight);
		mPreviewUseHW = true;
		mPreviewAfter = 1000000 / 100;
	}

	DBG_TIME_END("start camera", 1);
...
status_t V4L2CameraDevice::stopDevice()
{
	...
	char str[128];
	sprintf(str, "preview size : %dx%d", mFrameWidth, mFrameHeight);
	DBG_TIME_AVG_END(TAG_PREVIEW, str);
...
{
	DBG_TIME_BEGIN("wait for one frame ready", 0);
	...
	if (mTakingPicture)
	{
		DBG_TIME_END("capture an image", 0);
		pthread_cond_signal(&mTakePhotoCond);
		return false;
	}
	else
	{
		// preview video buffer, wait for queue empty
		if (mPreviewSwitch && mPreviewUseHW)
		{
			pthread_mutex_lock(&mQueueEmptyMutex);
			if (OSAL_GetElemNum(&mQueueBuffer) != 0)
			{
				pthread_cond_wait(&mQueueEmptyCond, &mQueueEmptyMutex);
			}
			pthread_mutex_unlock(&mQueueEmptyMutex);
...
#else
			memset((void*)mPreviewBuffer.buf_vir_addr[mPreviewBufferID], 0x10, mFrameWidth * mFrameHeight);
			memset((void*)mPreviewBuffer.buf_vir_addr[mPreviewBufferID] + mFrameWidth * mFrameHeight, 
				0x80, mFrameWidth * mFrameHeight / 2);
#endif
		
			v4l2_buf.addrPhyY		= mPreviewBuffer.buf_phy_addr[mPreviewBufferID]; 
			v4l2_buf.addrVirY		= mPreviewBuffer.buf_vir_addr[mPreviewBufferID]; 
			
			mCameraHAL->onNextFramePreview((void*)&v4l2_buf, v4l2_buf.format, mCurFrameTimestamp, this, true);

			pthread_mutex_lock(&mPreviewSwitchMutex);
			mPreviewSwitch = false;
			pthread_cond_signal(&mPreviewSwitchCond);
			pthread_mutex_unlock(&mPreviewSwitchMutex);
...
void V4L2CameraDevice::waitPreviewSwitch()
{
	pthread_mutex_lock(&mPreviewSwitchMutex);
	if (mPreviewSwitch && mPreviewUseHW)
	{
		pthread_cond_wait(&mPreviewSwitchCond, &mPreviewSwitchMutex);
	}
	pthread_mutex_unlock(&mPreviewSwitchMutex);
}
...
#else
	mCameraHAL->onTakingPicture((void*)mCurrentV4l2buf->addrVirY, this, false);
#endif
	mTakingPicture = false;

	char str[128];
	sprintf(str, "%s, picture size: %dx%d", (__HW_PICTURE__ == 1) ? "hw" : "sw", mFrameWidth, mFrameHeight);
	DBG_TIME_END(str, 0);
...
bool V4L2CameraDevice::previewThread()
{
	...
	if (pbuf == NULL)
	{
		pthread_cond_signal(&mQueueEmptyCond);
		pthread_mutex_unlock(&mQueueEmptyMutex);
	...
	DBG_TIME_AVG_AREA_IN(TAG_PREVIEW);
	...
	else
	{
		if (isPreviewTime())
		{
			mCameraHAL->onNextFramePreview((void*)pbuf, pbuf->format, mCurFrameTimestamp, this, false);
		}
	}
	
	DBG_TIME_AVG_AREA_OUT(TAG_PREVIEW);
...
int V4L2CameraDevice::getCurrentFaceFrame(void * frame)
{
	...
	if (mFaceDetectionEnable && mFaceDetectionStart)
	{
...
{
	return true;		// it's ok after modify the driver
